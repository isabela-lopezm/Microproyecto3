...
function loadFromLocalStorage(){ /*Esta funcion crea las filas, celdas para mostrar la info*/ 
    ...
    clientes.forEach(function(x,i) { /*Recorrer un vector-----función anonima. X almacena el objeto*/ 
...
function editFromLocalStorage(index){
    ...
    document.querySelector('#btn-guardar2').addEventListener("click",function(){
        ...
        clientes[index]["nombre"] = nomb.value; //Actualizar la información en la posicion especificada. 
...
function removeFromLocalStorage(index){
    ...
    localStorage.setItem(localStorageKeyName, JSON.stringify(clientes)); //Almacena todos nuevamente, menos el que borré. (se sobreescriben los datos, si tenia 4 ahora tendré3)
    loadFromLocalStorage();
}
